...
const Bestseller = () => {
    ...
    const {loading, data, error} = useFetch("http://localhost:1337/api/bestsellers?populate=*")
  ...
  return ( 
    <div className='bestsellers'>
        <div className="bestseller-container">
            ...
            <Slider {...settings}>
                        {goodData.map((item) => {
                            const imageURL = `http://localhost:1337${item.attributes.img.data[0].attributes.formats.thumbnail.url}`
                            return (
                                <div key={item.id}>
                                    <BestsellerCards
                                        key={item.id}
                                        ...
                                        discountedPrice={item.attributes.price}
        ...
        {goodData.map((product) => {
            const imageURL = `http://localhost:1337${product.attributes.img.data[0].attributes.formats.thumbnail.url}`
